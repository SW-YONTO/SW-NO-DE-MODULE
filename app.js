...
// Function to scan for node_modules directories
async function scanForNodeModules(startPath, maxDepth = 8, currentDepth = 0, progressCallback = null) {
    ...
    try {
        ...
        if (progressCallback) {
            progressCallback(`Scanning: ${startPath}`);
        }
            ...
            try {
                ...
                if (itemStats.isDirectory()) {
                    if (item === 'node_modules' && !isExcludedPath(itemPath)) {
                        ...
                        // Get size of node_modules directory
                        ...
                        const subDirs = await scanForNodeModules(itemPath, maxDepth, currentDepth + 1, progressCallback);
    ...
    console.log(`Found ${nodeModulesDirs.length} node_modules directories so far`);
    return nodeModulesDirs;
}
...
// Add a progress endpoint for real-time updates
app.get('/scan-progress/:sessionId', (req, res) => {
    res.writeHead(200, {
...
// Enhanced scan with progress
app.post('/scan-with-progress', async (req, res) => {
    try {
        const { scanPath, sessionId } = req.body;
        let pathToScan = scanPath || 'd:\\';
        
        // Normalize the path
        pathToScan = path.resolve(pathToScan);
        
        console.log(`Starting scan with progress from: ${pathToScan}`);
        ...
        const progressCallback = (message) => {
            if (global.progressSessions && global.progressSessions.has(sessionId)) {
                ...
                try {
                    progressRes.write(`data: ${JSON.stringify({ type: 'progress', message })}\n\n`);
                } catch (err) {
                    console.log('Progress update failed:', err.message);
                }
            }
        };
        
        const nodeModulesDirs = await scanForNodeModules(pathToScan, 8, 0, progressCallback);
        ...
        if (global.progressSessions && global.progressSessions.has(sessionId)) {
            const progressRes = global.progressSessions.get(sessionId);
            try {
                progressRes.write(`data: ${JSON.stringify({ type: 'complete', count: nodeModulesDirs.length })}\n\n`);
                progressRes.end();
                global.progressSessions.delete(sessionId);
            } catch (err) {
                console.log('Completion message failed:', err.message);
            }
        }
...
// Test endpoint to verify scanning logic
